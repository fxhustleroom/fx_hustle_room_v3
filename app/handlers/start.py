...
async def start_handler(message: Message, state: FSMContext, session: AsyncSession) -> None:
    try:
        ...
        language_code = TELEGRAM_TO_SUPPORTED.get(
            (message.from_user.language_code or "").split("-")[0],
        ...
        await message.answer(
            t("select_language", language_code),
            reply_markup=language_keyboard(),
        )

    except Exception as exc:
        logger.exception(f"Error in /start handler: {exc}")
...
async def language_selected(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    try:
        ...
        logger.exception(f"Error in language_selected handler: {exc}")
